			...
			angular
				...
				.controller('MwlCalendarCtrl', ["$scope", "$log", "$timeout", "$attrs", "$locale", "moment", "calendarTitle", "calendarHelper", function ($scope, $log, $timeout, $attrs, $locale, moment, calendarTitle, calendarHelper) {
					...
					function refreshCalendar() {
						...
						if (
							previousDate.clone().startOf(vm.view).isSame(currentDate.clone().startOf(vm.view)) && !previousDate.isSame(currentDate) &&
...
//									event.left = bucketIndex * 100;
//									buckets[bucketIndex].push(event);
//								}

							});
							
							var columnIndex = parseInt(event.type.split("-")[1]);
							var larguraDay = columnIndex*100;
							var larguraCalendar = $('.cal-day-box').width() - 50;
							
							while (buckets.length < columnIndex) {
								buckets.push([]);
							}
							
							if (larguraDay > larguraCalendar) {
								$('.cal-day-box').css('width', larguraDay + 80 + "px");
							};
							
							event.left = (columnIndex - 1) * 100;
							buckets[columnIndex-1].push(event);
								
//							if (cannotFitInABucket) {
//								event.left = buckets.length * 100;
//								buckets.push([event]);
//							}
		...
		/***/ function (module, exports) {
			...
			if (typeof __WEBPACK_EXTERNAL_MODULE_48__ === 'undefined') {
