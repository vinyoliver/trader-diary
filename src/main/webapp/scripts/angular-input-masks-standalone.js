	...
	return {
		...
		link: function(scope, element, attrs, ctrl) {
			...
			function parser(value) {
				...
				var formatedValue;
				if(value.indexOf(',') > 0 && viewMask.apply(valueToFormat).indexOf(',') === -1){
					valueToFormat = valueToFormat.substr(0,valueToFormat.length-2);
				}else if(value.indexOf(',') === -1 && viewMask.apply(valueToFormat).indexOf(',') != -1 && ctrl.$modelValue){
